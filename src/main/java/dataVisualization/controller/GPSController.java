...
public class GPSController {
	...
	@ResponseBody
	public Object getRecordsByCode(HttpServletRequest request) throws ParseException {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String gpstime = request.getParameter("gpstime");
//		String code = request.getParameter("code");
		List<Record> records = new ArrayList<Record>();
		//从service获取records
		records = recordService.getRecords(gpstime);
//		records = recordService.getRecordsByCode(gpstime);
		...
		for (int i = 0; i < length; i++) {
			...
			String carcode = record.getCode();
			Long date = format.parse(record.getGpstime()).getTime();
			//这是该对象的一个属性
			...
			name1.put("name", carcode );
			name1.put("value", date);
			//name1.put("value", date);
			//name2.put("value", date );
...
//			sequence.add(name2);
			...
			//字典
			coordinates.put(carcode, coordinate);
		}
